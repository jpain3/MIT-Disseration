...
list.of.packages <- c("optimParallel","optimx")
...
BeerGame_Optimization_with_History <- function(Agent_Parameter = FALSE, Agent_Parameter_History = FALSE, Agent_Order_History = FALSE, AI_Entity_Index = FALSE, 
  ...
  for (t in 2:(horizon+1)) {
    ...
    if (AI_Entity_Index != FALSE & (BaseStockAgent != TRUE)) {
  ...
  if (Mode == "Calibration" || Mode == "Outputs"){
    ...
    if (Reward == "Amp") {
      SimReward = Trimmed_total_amp_reward
    } else {
      if (Myopic_Reward == TRUE) {
        SimReward = Weighted_Myopic_Total_Costs
      } else {
...
Myopic_Reward = FALSE    #IF false, the agent considers the full team cost when optimizing
...
EstSterman89 = TRUE
EstBaseStock = FALSE
EstSafety_Stock = NA #If BaseStock = TRUE and Safety_Stock = NA, then safety stock is assumed to be the initial inventory from above
EstOliva21 = FALSE
EstOlivaModel = "Model3" #choices here are Model1 Model2 Model3 Model4
EstStermanDogan15 = FALSE

## DEFINE AGENT TYPE
BaseStockAgent = FALSE #If not set to True, then assumes a Sterman '89 style model for the agent.
...
#   when simulating and optimizing for the future. Otherwise, if set to FALSE, the agent is assumed to have full knowledge of the real orders
...
if (BaseStock == TRUE || Oliva21 == TRUE || StermanDogan15 == TRUE) {
  Team_Index_Vector = c(1)
}

if (BaseStockAgent == TRUE) {
  Backward_Calibration = FALSE
  Forward_Optimization = FALSE
  axial_improvement_search = FALSE
}
...
for (Team_Index in Team_Index_Vector) {
  ...
  if (GreedyStepAgent != TRUE) {
    ...
    #create a dummy agent order parameter array for later comparison
    if (Sterman89 == TRUE){
      No_Agent_Parameters = unlist(Original_True_Parameter_df[AI_Entity_Index,])
      rownames(No_Agent_Parameters) = NULL
      No_Agent_Parameter_History = Original_Agent_Parameter_History
      No_Agent_Parameter_History[,1:ncol(No_Agent_Parameter_History)] = unname(No_Agent_Parameters)
    }
  ...
  for (time_step in 1:(Sim_horizon)) {
    ...
    if ((Forward_Optimization != FALSE) & (Skip_Optimization_Flag == FALSE)) {
      ...
      if (DebugFlag == TRUE) {
        ...
        Test_Myopic_Reward = TRUE
        
        DebugTest = BeerGame_Optimization_with_History(Agent_Parameter = Opt_Initial_Guess, 
                                   Agent_Parameter_History = Agent_Parameter_History,
                                   Agent_Order_History = Agent_Order_History,
                                   AI_Entity_Index = AI_Entity_Index,
                                   BaseStockAgent = BaseStockAgent,
                                   ...
                                   Integer_Ordering = Integer_Ordering,
                                   Sterman89 = EstSterman89, Oliva21 = EstOliva21, OlivaModel = EstOlivaModel, 
                                   StermanDogan15 = EstStermanDogan15, #Assumes Sterman89 model when optimizing
                                   BaseStock = EstBaseStock, Safety_Stock = EstSafety_Stock, 
                                   ...
                                   Myopic_Reward = Test_Myopic_Reward,
                                   Selfweight = Selfweight)
      }
                                 
      
      #Opt_Initial_Guess = c(.36,.26,.34,17)
  ...
  #   with the initial Pre-optimized but fixed agent in place
  Reset_List = reset_game(Shipping_Delay = Shipping_Delay, Information_Delay = Information_Delay, 
                          Initial_Inventory = Initial_Inventory, horizon = Sim_horizon
  )
  list2env(Reset_List, envir = .GlobalEnv)
  Default_Agent_Performance = BeerGame_Optimization_with_History(Agent_Parameter = Original_Agent_Parameter_History[,Sim_horizon],
  ...
  OUTPUT_Agent_Parameter_History = Agent_Parameter_History[,1:time_step]
